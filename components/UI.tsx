...
export function H1(props) {
  const { children, className } = props;
  return <h1 className={className + " mt-10 text-4xl font-black"}>{children}</h1>
}
...
export function TheCodeBlock(props) {
  ...
  const isMultiline = props.children.split("\n").filter(line => line !== "").length > 1;
...
export function MdLi(props) {
  ...
  return (
    <li className="ml-6 list-disc list-outside list-item">{children}</li>
  )
}